...
Python 3.10+; standard library only; fully offline.
...
The top/bottom column names and f/x rule are transcribed from the photographs.
Vig support is an addition: VERIFY its two column names against your CSV.
...
CSV_FILES = [
    ...
    # Path(r"C:\path\to\another\evaluation_results.csv"),
]
OUTPUT_ROOT = Path(r"C:\Users\laka\software\good_bad_split_94_new")
...
TASK_COLUMNS = {
    ...
    "vig": {
        # ASSUMED names, not visible in the screenshots: adjust if necessary.
        "labels": ("Quality_VI_Vig_Label",),
        "sources": ("Quality_VI_Vig_Sources",),
    },
}
...
def copy_plan(plan, output_root):
    ...
    for task, label, source, _ in plan:
        destination = output_root / task / label / source.name
        key = str(destination).casefold()
        if key in used:
            digest = hashlib.sha256(str(source).encode("utf-8")).hexdigest()[:12]
            destination = destination.with_name(f"{source.stem}__{digest}{source.suffix}")
            number = 2
            while str(destination).casefold() in used:
                destination = destination.with_name(f"{source.stem}__{digest}_{number}{source.suffix}")
                number += 1
...
def main():
    ...
    try:
        ...
        if args.dry_run:
            ...
            counts = copy_plan(plan, output_root)
